...
public interface DocMapper {
    ...
    @Update("update orders set state = '完成' where doc_id = #{id}")
...
//    医生给病人提交出院申请
    ...
    @Select("SELECT * FROM hospitalstay WHERE state = '住院' AND adm_id in (SELECT id FROM admission WHERE doc_id = #{id})")
    @Results({
            @Result(column = "id", property = "id"),
